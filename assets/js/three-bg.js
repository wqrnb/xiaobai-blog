...
(function () {
  ...
  var frames = [], frameMeshes = [], frameIndex = {}, texturePromises = {};
  ...
  /* 真实作品画框：把 A 的 74 件作品贴图悬浮在星球表面/周围轨道 */
  function ensureFrameTexture(frame) {
    if (frame.texture) return Promise.resolve(frame.texture);
    var id = frame.item.id;
    var data = window.SITE_TEXTURES && window.SITE_TEXTURES[id];
    if (!data) {
      if (!texturePromises[id]) {
        texturePromises[id] = new Promise(function (resolve, reject) {
          var sc = document.createElement('script');
          sc.src = 'assets/textures/' + id + '.js';
          sc.onload = resolve;
          sc.onerror = reject;
          document.head.appendChild(sc);
        });
      }
      return texturePromises[id].then(function () { return ensureFrameTexture(frame); });
    }
    var loader = new THREE.TextureLoader();
    return new Promise(function (resolve) {
      loader.load(data, function (tex) {
        tex.encoding = THREE.sRGBEncoding;
        tex.anisotropy = Math.min(4, renderer.capabilities.getMaxAnisotropy());
        frame.texture = tex;
        frame.plane.material.map = tex;
        frame.plane.material.color.set(0xffffff);
        frame.plane.material.needsUpdate = true;
        resolve(tex);
  ...
  function buildFrames() {
    ...
    var featuredSet = {};
    (data.featuredIds || []).forEach(function (id) { featuredSet[id] = true; });
    var groups = { xhs: [], bili: [], featured: [] };
    items.forEach(function (it) {
      if (featuredSet[it.id]) groups.featured.push(it);
      else if (it.platform === 'xhs') groups.xhs.push(it);
      else if (it.platform === 'bili') groups.bili.push(it);
    });
    var counts = { xhs: 0, bili: 0, featured: 0 };
    Object.keys(groups).forEach(function (key) {
      var list = groups[key];
      var base = VIEWS[key] || VIEWS.featured;
      list.forEach(function (item) {
        var idx = counts[key]++;
        var ring = Math.floor(idx / 6);
        var col = idx % 6;
        var latOff = (col - 2.5) * 0.16;
        var lonOff = (ring - 1) * 0.17 + (col - 2.5) * 0.02;
        var radius = 10.4 + ring * 1.15;
        var theta = base.theta + lonOff;
        var phi = Math.max(0.55, Math.min(2.1, base.phi + latOff * 0.9));
        var dir = dirVec(theta, phi);
        var pos = dir.clone().multiplyScalar(radius);
        var group = new THREE.Group();
        group.position.copy(pos);
        group.lookAt(pos.clone().multiplyScalar(2));
        group.rotateY((idx % 3 - 1) * 0.08);
        var borderColor = key === 'bili' ? 0x7bccff : (key === 'featured' ? 0xeaa1e1 : 0xff92c7);
        var border = new THREE.Mesh(
          new THREE.BoxGeometry(1.72, 1.28, 0.12),
          new THREE.MeshBasicMaterial({ color: borderColor, transparent: true, opacity: 0.85 })
        );
        var plane = new THREE.Mesh(
          new THREE.PlaneGeometry(1.52, 1.08),
          new THREE.MeshBasicMaterial({ color: 0x6a3b62, transparent: true, opacity: 0.92 })
        );
        plane.position.z = 0.075;
        plane.userData.frameId = item.id;
        group.add(border);
        group.add(plane);
        planetGroup.add(group);
        var frame = {
          item: item, group: group, plane: plane, view: key,
          basePos: pos.clone(), phase: Math.random() * Math.PI * 2,
          loaded: false, texture: null
        };
        frames.push(frame);
        frameMeshes.push(plane);
        frameIndex[item.id] = frame;
      });
    });
    // 加载当前分区附近的一小部分画框，开机即有内容可看
    if (!IS_SMALL) {
      frames.slice(0, 8).forEach(function (f) { ensureFrameTexture(f); });
    }
  }
  ...
  function pickFrame(e) {
    ...
    var ray = new THREE.Raycaster();
    ray.setFromCamera(ndc, camera);
    var hits = ray.intersectObjects(frameMeshes, false);
    if (!hits.length) return null;
    var blocked = ray.intersectObject(surfaceMesh, false);
    if (blocked.length && blocked[0].distance < hits[0].distance - 0.02) return null;
    return hits[0].object.userData.frameId || null;
  }
  function onTap(e) {
    if (orbit.moved > 8) return;               // 拖拽不算点击
    var fid = pickFrame(e);
    if (fid) {
      var f = frameIndex[fid];
      if (f) ensureFrameTexture(f);
      dispatch('xwb:frame', { id: fid });
      return;
    }
    var view = pickNode(e);
  ...
  function animate() {
    ...
    frames.forEach(function (f) {
      var bob = Math.sin(t * 0.85 + f.phase) * 0.14;
      f.group.position.copy(f.basePos).add(f.basePos.clone().normalize().multiplyScalar(bob));
      if (f.highlight) f.highlight = Math.max(0, f.highlight - dt * 0.35);
      var sc = 1 + (f.highlight || 0) * 0.22;
  ...
  api.focusItem = function (id) {
    var f = frameIndex[id];
    if (!f) return false;
    api.goTo(f.view);
    f.highlight = 1;
    setTimeout(function () { f.highlight = 0; }, 2600);
    return true;
  };
  api.pulse = function () {
  ...
  if (!api.supported) return;
  api.debug = {
    renderer: renderer, scene: scene, camera: camera, anchors: anchors, frames: frames, pickFrame: pickFrame, pickNode: pickNode,
