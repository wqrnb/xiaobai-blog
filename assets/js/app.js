...
(function () {
  ...
  function renderSearchResults(q) {
    ...
    $$('#search-results li[data-id]').forEach(function (li) {
      li.addEventListener('click', function () { var id = li.getAttribute('data-id'); var f = findItem(id); closeSearch(); if (window.XBW_BG && window.XBW_BG.focusItem) window.XBW_BG.focusItem(id); if (f) openDetail(f.item, f.kind); });
    });
  }
  ...
  /* ---------- 主题 / 音乐 ---------- */
  function savedTheme() {
    try { return localStorage.getItem('xiaobai-blog-theme') || 'day'; } catch (e) { return 'day'; }
  }
  function saveTheme(t) {
    try { localStorage.setItem('xiaobai-blog-theme', t); } catch (e) {}
  }
  function applyThemeButton(t) {
    var dark = t === 'night';
    document.documentElement.setAttribute('data-theme', dark ? 'dark' : '');
    $('#theme-btn').innerHTML = dark ? icon('moon') : icon('sun');
    if (window.XBW_BG && window.XBW_BG.setTheme) window.XBW_BG.setTheme(t);
  }
  function toggleTheme() {
    var next = document.documentElement.getAttribute('data-theme') === 'dark' ? 'day' : 'night';
    applyThemeButton(next);
    saveTheme(next);
  }
  var musicAudio = null, musicBusy = false;
  function toggleMusic() {
    ...
    if (!musicAudio) {
      musicAudio = new Audio('assets/audio/starlit-steps.wav');
      musicAudio.loop = true;
      musicAudio.volume = .45;
      musicAudio.preload = 'none';
    }
    musicBusy = true;
    var ret = musicAudio.play();
    if (ret && ret.then) {
      ret.then(function () { musicBusy = false; btn.classList.add('playing'); })
         .catch(function () { musicBusy = false; btn.classList.remove('playing'); });
    } else {
      musicBusy = false;
  ...
  function bindUI() {
    ...
    document.addEventListener('xwb:frame', function (e) {
      var id = e.detail && e.detail.id;
      if (!id) return;
      var entry = allItems.filter(function (x) { return x.item.id === id; })[0];
      if (entry) openDetail(entry.item, entry.kind);
    });
    document.addEventListener('visibilitychange', function () {
      if (document.hidden && musicAudio && !musicAudio.paused) { musicAudio.pause(); $('#music-btn').classList.remove('playing'); $('#music-btn').innerHTML = icon('music'); }
    });
  }
  ...
  function start() {
    if (!DATA) return;
    if (/[?&]view=2d/.test(location.search)) {
      document.body.classList.add('xwb-2d');
      if (window.XBW_BG && window.XBW_BG.debug && window.XBW_BG.debug.renderer) {
        window.XBW_BG.debug.renderer.domElement.style.display = 'none';
      }
    }
    applyThemeButton(savedTheme());
    buildBackground();
    bindUI();
    setupFxCanvas();
    setupBeam();
